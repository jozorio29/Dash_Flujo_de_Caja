...
export function IncomeStatementView() {
  ...
  async function save() {
    ...
    let payload;
    try {
      payload = renaming ? { action: 'rename', kind: renaming.kind, id: renaming.id, previousName: renaming.name, name } : newSubgroup ? { action: 'group', sectionId: newSubgroup.section_id, parentId: newSubgroup.id, name } : newSection ? { action: 'group', sectionId: newSection.id, name } : editing ? { action: 'value', accountId: editing.account.id, month: editing.month, year, currency, previous: editing.previous, amount: draft.trim() ? normalizeAmount(draft) : null } : { action: 'account', groupId: newGroup?.id, name };
    ...
    try {
      ...
      setData(previous => {
        ...
        if (newSubgroup || newSection) return { ...previous, groups: [...previous.groups, body.group] };
