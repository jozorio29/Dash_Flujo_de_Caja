...
export function IncomeStatementView() {
  ...
  return <div className="space-y-3 p-4 md:p-6 lg:p-8">
    <header className="flex flex-col justify-between gap-4 xl:flex-row xl:items-start"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-blue-600"><FileSpreadsheet className="h-4 w-4" />Reportes financieros</div><h1 className="text-2xl font-bold tracking-tight text-slate-900">Estado de Resultados</h1><p className="mt-2 text-sm text-slate-500">Europe Intelligence Solutions · Sucursal Bolivia</p></div><div className="flex flex-col items-start gap-2 xl:items-end"><span className="rounded-full border bg-white px-3 py-1.5 text-xs text-slate-600">{loading ? 'Cargando…' : data ? (data.canEdit ? 'Edición habilitada' : 'Solo lectura') : 'Sin conexión'}</span>
    <div className="flex flex-wrap items-end gap-2 xl:justify-end"><div className="flex gap-2">
      <label className="text-xs text-slate-500">Ejercicio<select className="mt-1 block rounded-lg border bg-white px-2 py-2 text-xs text-slate-900" value={year} disabled={modal} onChange={e => { setYear(Number(e.target.value)); setNotice(''); }}>{[2025, 2026, 2027].map(y => <option key={y}>{y}</option>)}</select></label>
      <label className="text-xs text-slate-500">Moneda<select className="mt-1 block rounded-lg border bg-white px-2 py-2 text-xs text-slate-900" value={currency} disabled={modal} onChange={e => { setCurrency(e.target.value); setNotice(''); }}><option value="USD">USD · Dólares</option><option value="BOB">BOB · Bolivianos</option></select></label></div>
      <div className="flex flex-wrap gap-2"><button className={button} disabled={saving || moving || Boolean(renaming)} onClick={() => setCollapsed(new Set())}>Expandir todo</button><button className={button} disabled={modal} onClick={() => setCollapsed(new Set(data?.groups.map(g => g.id)))}>Contraer todo</button><button className={button} disabled={loading || modal} onClick={() => { setRevision(r => r + 1); setNotice(''); }}><RefreshCw className="mr-1 inline h-3 w-3" />Actualizar</button></div></div></div></header>
