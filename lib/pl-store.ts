...
export async function readPL(year: number, currency: string) {
  const [sections, groups, accounts, values] = await Promise.all([
    ...
    allRows<PLValue>('pl_monthly_values', `select=id,account_id,year,month,currency,amount::text,updated_at&year=eq.${year}&currency=eq.${currency}&order=id`),
  ]);
  return { sections, groups, accounts, values };
}
